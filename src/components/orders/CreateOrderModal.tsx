...
import { PurchaseOrderItem, PurchaseOrder } from '../../types'; // Importamos PurchaseOrder
import { warehouses, EXCHANGE_RATE } from '../../data/mockData';
import { format } from 'date-fns';
import { es } from 'date-fns/locale';

// Interfaz del proveedor mapeada desde la DB 
...
// Usamos PurchaseOrder ya que contiene todos los campos necesarios
interface InitialOrderData extends PurchaseOrder {} 
...
  initialData?: InitialOrderData | null; // Datos para Duplicar o Editar
  isEditMode?: boolean; // Propiedad para Edición
}

interface OrderFormData {
  id?: string; // Incluimos el ID para el modo edición
  supplierRuc: string;
  supplierName: string;
  warehouse: string;
...
// Función para inicializar el estado (ya sea vacía, duplicada o editada)
const getInitialState = (currency: 'PEN' | 'USD', initialData?: InitialOrderData | null, isEditMode?: boolean): OrderFormData => {
    if (initialData) {
        const todayFormatted = format(new Date(), 'yyyy-MM-dd');
        
        // Modo EDICIÓN: Copiar todos los datos tal cual, incluyendo el ID.
        if (isEditMode) {
            return {
                id: initialData.id,
                supplierRuc: initialData.supplierRuc,
...
        // Modo DUPLICAR: Copiar datos, pero generar nuevos IDs para los ítems y asegurar la fecha.
        return {
            ...initialData,
            id: undefined, // Aseguramos que no se envíe el ID antiguo
            // Corregido: Usar objetos Date para la comparación segura
            estimatedDelivery: new Date(initialData.estimatedDelivery) > new Date() 
...
        };
    }
    // Estado por defecto (Nueva Orden)
    return {
        id: undefined,
        supplierRuc: '',
        supplierName: '',
...
const CreateOrderModal: React.FC<CreateOrderModalProps> = ({ currency, onClose, onSubmit, allSuppliers, initialData, isEditMode = false }) => {
  
  // Usar la función getInitialState para inicializar formData
  const [formData, setFormData] = useState<OrderFormData>(() => 
      getInitialState(currency, initialData, isEditMode)
  );

  // Ajustar el paso inicial. Si es edición o duplicación (tiene datos iniciales), salta a 'details'.
  const [currentStep, setCurrentStep] = useState<'supplier' | 'details' | 'items' | 'review'>(
      formData.supplierRuc !== '' ? 'details' : 'supplier'
  );
...
  // Filtra la lista de proveedores REALES de la DB para la búsqueda
  const filteredSuppliers = allSuppliers.filter((supplier: SupplierData) =>
...
    
    // Obtenemos la categoría (sector) del proveedor para enviarla al backend
    const supplierCategory = allSuppliers.find(s => s.ruc === formData.supplierRuc)?.sector || 'General';

    // Datos que se envían al API (POST para creación/duplicación, PUT para edición)
    const orderData = {
        // Incluir ID si estamos en modo edición
...
      igv: totals.igv,
      total: totals.total,
      // Mapeamos los ítems, omitiendo el ID temporal si es necesario
...
              {isEditMode ? 'Editar Orden de Compra' : (initialData ? 'Duplicar Orden de Compra' : 'Nueva Orden de Compra')}
            </h2>
...
              {/* Renderizar la lista de proveedores de la DB (allSuppliers) */}
...
                      <p className="text-sm text-gray-600 mb-2">{supplier.contact.address || 'Sin dirección registrada'}</p>
...
                  disabled={!!initialData} // Deshabilitar si es Duplicar y ya tiene proveedor
                >
                  ← Volver
                </button>
                <button
...
                        Ingrese Cantidad y Precio Unitario para ver el total.
                    </div>
                )}
              </div>
...
                <div>
                  <h4 className="font-medium text-gray-700 mb-4">Items Agregados ({formData.items.length})</h4>
                  
...
                >
                  {isEditMode ? 'Guardar Cambios' : (initialData ? 'Duplicar y Crear Orden' : 'Crear Orden de Compra')}
                </button>
              </div>
            </div>
          )}
